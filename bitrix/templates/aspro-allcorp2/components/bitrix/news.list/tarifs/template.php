												...
												<div class="item" id="<?=$this->GetEditAreaId($arItem['ID']);?>" <?=($bOrderViewBasket ? ' data-item="'.$dataItem.'"' : '')?>>
													<div class="body-info">
														...
														<?if($arItem['PROPERTIES']['PRICE']['VALUE']):?>
															<div class="price-block <?=($arItem['PROPERTIES']['PRICEOLD']['VALUE'] ? 'w-old' : '');?>"><?=$arItem['PROPERTIES']['PRICE']['VALUE'];?></div>
														<?endif;?>
														<?if($arItem['PROPERTIES']['PRICEOLD']['VALUE']):?>
															<div class="price-block"><?=$arItem['PROPERTIES']['PRICEOLD']['VALUE'];?></div>
														<?endif;?>
														...
														<?if($arItem['PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES' && !$bOrderViewBasket):?>
															<div class="order<?=($bOrderViewBasket ? ' basketTrue' : '')?>">
																<?if($arItem['PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES' && !$bOrderViewBasket):?>
																	<span class="btn btn-default btn-xs animate-load" data-event="jqm" data-param-id="<?=CAllcorp2::getFormID("aspro_allcorp2_order_product");?>" data-name="order_product" data-autoload-product="<?=CAllcorp2::formatJsName($arItem['NAME']);?>"><?=(strlen($arParams['S_ORDER_PRODUCT']) ? $arParams['S_ORDER_PRODUCT'] : GetMessage('S_ORDER_PRODUCT'))?></span>
																<?endif;?>
															</div>
														<?endif;?>
													</div>
