...
<div class="catalog item-views list <?=($arParams["IMAGE_POSITION"] ? "image_".$arParams["IMAGE_POSITION"] : "")?> <?=($templateName = $component->{"__parent"}->{"__template"}->{"__name"})?>">
	<?// top pagination?>
	...
	<?if($arResult["ITEMS"]):?>
		<div class="items row">
			...
			<?foreach($arResult["ITEMS"] as $i => $arItem):?>
				...
				<?ob_start();?>
					<?if($bImage):?>
						<div class="image <?=($bImage ? ' w-picture' : ' wo-picture')?>">
							<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
