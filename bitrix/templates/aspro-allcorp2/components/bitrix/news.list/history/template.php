...
<div class="item-views list <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
	...
	<div class="row items">
		<?foreach($arResult['ITEMS'] as $arItem):?>
			...
			$arImage = ($bImage ? CFile::ResizeImageGet($nImageID, array('width' => 220, 'height' => 220), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : array());
			...
			<?ob_start();?>
				<?if($bImage):?>
					<div class="image">
						<?if($arItem['FIELDS']['DETAIL_PICTURE'] && $arItem['DETAIL_PICTURE']['SRC']):?><a href="<?=$arItem['DETAIL_PICTURE']['SRC']?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-inside fancybox"><?endif;?>
						...
						<?if($arItem['FIELDS']['DETAIL_PICTURE'] && $arItem['DETAIL_PICTURE']['SRC']):?><span class="zoom"></span></a><?endif;?>
					</div>
				<?endif;?>
