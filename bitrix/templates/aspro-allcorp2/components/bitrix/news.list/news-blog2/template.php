			...
			<?foreach($arResult['ITEMS'] as $arItems):?>
				<div class="items row flexbox">
					...
					<?foreach($arItems['ITEMS'] as $key => $arItem):?>
						...
						<?endif;?>
							<div class="s_<?=$count;?> <?=((isset($arItem['CLASS']) && $arItem['CLASS']) ? $arItem['CLASS'] : 'col-md-4 col-sm-4');?>">
								<div class="item shadow animation-boxs <?=($isWideBlock ? 'wide-block' : '')?> <?=($hasWideBlock ? '' : 'normal-block')?>"  id="<?=$this->GetEditAreaId($arItem['ID']);?>">
									<div class="inner-item <?=($bActiveDate ? 'with-date' : '');?>">
										<?if($bImage):?>
										...
										<div class="title">
											...
											<?if($arItem['DISPLAY_ACTIVE_FROM']):?>
												<?if(strlen($arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE'])):?>
													<div class="date-block"><?=$arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE']?></div>
												<?else:?>
