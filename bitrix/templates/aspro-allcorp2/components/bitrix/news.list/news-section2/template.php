	...
	<?if($arResult["SECTIONS"]):?>
		...
		<div class="items row">
			<?foreach($arResult["SECTIONS"] as $i => $arSection):?>
				...
				<?endif;?>
				<?if(isset($arSection["UF_TOP_SEO"]) && $arSection["UF_TOP_SEO"]):?>
					<div class="text-section"><?=$arSection["UF_TOP_SEO"];?></div>
				<?endif;?>
					<div class="row">
				<?// show section items?>
					...
					<?ob_start();?>
						...
						<?if($bDetailLink):?>
							<div class="link-block-more">
								<a href="<?=$arItem['DETAIL_PAGE_URL']?>" <?=(strpos($arItem['DETAIL_PAGE_URL'], 'http') !== false ? 'target="_blank"' : "")?> class="btn btn-default btn-sm"><?=GetMessage('TO_ALL')?></a>
							</div>
						<?endif;?>
