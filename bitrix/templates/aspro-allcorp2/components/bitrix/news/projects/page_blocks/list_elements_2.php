<div class="mixitup-container">
	...
	$arParentSections = CCache::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arFilter, false, $arSelect);
	if($arParentSections)
	{
		...
		<div class="head-block top controls 111">
			...
			<div class="item-link <?=($bHasSection ? '' : 'active');?>">
				...
				<?endif;?>
						<span class="btn-inline black"><?=GetMessage('ALL_PROJECTS');?></span>
				<?if($bHasSection):?>
					</a>
				<?else:?>
					</div>
			...
			<?foreach($arParentSections as $arParentItem):?>
				...
				<div class="item-link <?=($bSelected ? 'active' : '');?>">
					<?if(!$bHasSection):?>
						<div class="title btn-inline black" data-filter=".s-<?=$arParentItem['ID']?>">
					...
					<?else:?>
						...
						<?else:?>
							<a class="title btn-inline black" href="<?=$arParentItem['SECTION_PAGE_URL'];?>">
						<?endif;?>
								<span class="btn-inline black"><?=$arParentItem['NAME'];?></span>
						<?if($bSelected):?>
							</div>
