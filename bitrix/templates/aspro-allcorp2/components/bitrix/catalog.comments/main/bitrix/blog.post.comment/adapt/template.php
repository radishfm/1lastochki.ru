	...
	else
	{
					...
					if($arResult["use_captcha"]===true)
					{
						...
						*/?>
						<div class="captcha-row clearfix form form-group">
							<label><span><?=GetMessage("B_B_MS_CAPTCHA_SYM")?> <span class="required-star">*</span></span></label>
							<div class="captcha_image">
								<img src="/bitrix/tools/captcha.php?captcha_sid=<?=htmlspecialcharsbx($arResult["CaptchaCode"])?>" class="captcha_img" border="0" />
								<input type="hidden" name="captcha_code" id="captcha_code" class="captcha_sid" value="<?=htmlspecialcharsbx($arResult["CaptchaCode"])?>" />
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-content">
						...
						?>
						<span class="blog-comment-link"><a href="#c<?=$comment["ID"]?>"><?=GetMessage("B_B_MS_LINK")?></a></span>
						<?
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="btn btn-default btn-transparent-bg" href="javascript:void(0)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
				<a name="0"></a>
				<?
