...
<?global $APPLICATION;
$arThemePopup = CAllcorp2::GetFrontParametrsValues(SITE_ID);
$url = '';
if(isset($_GET['url']) && $_GET['url'])
	$url = htmlspecialchars($_GET['url']);?>

<div class="form popup v_POPUP">
<div class="wrap">
	...
	<?$APPLICATION->IncludeComponent(
		...
		strtolower($arThemePopup["REGIONALITY_VIEW"]),
		Array(
			"URL" => $url,
			"POPUP" => "Y",
	...
	<script type="text/javascript">
		...
		if($("#search").length)
		{
			if(arAllcorp2Options['THEME']['REGIONALITY_SEARCH_ROW'] == 'Y')
			{
				$("#search").autocomplete({
					minLength: 2,
					source: function(request, response){
						console.log(request)
						$.getJSON( arAllcorp2Options['SITE_DIR']+'ajax/city_select.php', {
				            term: request.term,
				            url: '<?=$url;?>'
				          }, response );
					...
					select: function(event, ui) {
						...
						if(arAllcorp2Options['SITE_ADDRESS'].indexOf(',') != '-1')
						{
							...
							$.cookie('current_region', ui.item.ID, {path: '/',domain: arAllcorp2Options['SITE_ADDRESS']});
